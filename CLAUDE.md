...
**Read the index before concluding a task has no skill.** `~/.claude/SKILL-INDEX.md` catalogs every skill on this machine with full descriptions — personal skills under their own name, and every skill from an installed plugin under the name that invokes it (`plugin:skill`) — grouped by visibility: always listed, path-gated, invocable by name, and disabled. Most are invisible in your session listing and run anyway when invoked as `/name`; only the Disabled group needs `settings.json` changed first. Absence from your listing means nothing about availability, and the index carries the current counts.
...
A subject is read cold, in `git log`, by someone who never saw the session, so every noun in it has to mean something to that reader. A word this project invented (a mode's setting, a hook's verdict, a probe's variant) is replaced by the plain thing it means, unless it is a file, command, or symbol the reader can grep, and even then it usually wants a plain word beside it. "Record reviews, stands and holds in crew-record" fails, because three of its nouns exist only in this repo's vocabulary; "Log reviewer verdicts in the subagent run record" passes. The linter cannot tell a coined word from an ordinary one, so this check is yours.
