...
36-44% when the rule is buried and 8-15% when it is early, and so burial makes a
rule obeyed *less often than having no rule at all* (bare: 21%).
...
- **The penalty is specific to trivial changes.** Partly. Trivial +29.2pp
  (p=0.002), substantial +16.7pp (p=0.059), larger on small changes and present in
...
else below. Nothing is ever dropped, which removes the system's main risk of a
