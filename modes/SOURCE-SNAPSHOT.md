<!--
  Provenance snapshot: CLAUDE.md as it stood immediately before the rule
  corpus was cut out of it, on 2026-09-07.

  corpus-fidelity checks every rule against this file, not against the live
  CLAUDE.md. Once the rules move into modes/rules/ the live file no longer
  contains them, so it can no longer answer 'did this rule drift from what
  the operator actually wrote'. This can, permanently.

  Do not edit. Regenerate only when deliberately re-basing the corpus.
-->
