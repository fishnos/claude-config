...
## Voice and Venue (read before drafting a single sentence)

Two layers sit above this skill body and override it on conflict.

**1. Venue overrides the numbers.** Every word budget, abstract length, and
section template below was written for philosophy preprints with no page limit.
If the target venue is ICRA, IROS, RA-L, or any IEEE robotics conference, read
`~/.claude/skills/academic-paper-strategist/references/venue_icra.md` first and
use its budgets and structure instead. It also carries the double-anonymous
checklist and the IEEE AI-disclosure policy, which governs whether this skill
may draft a given section at all.

**2. `daniel-voice` owns the sentences.** This skill owns structure, section
order, quality gates, and the evaluation scripts. It does **not** own register.
Load the `daniel-voice` skill before writing prose and apply it as a
*constraint, not a register*:

- **Apply** the flow rule above all else: each sentence opens on what the
  previous sentence established and closes on what is new. Apply the mechanics
  and the connective set. Apply the ban on hollow closers, inflated
  significance, and aspirational endings.
- **Suppress** the personal register: no narrative opener before the thesis, no
  first-person singular, no reflective turn. Academic convention governs
  diction, hedging, and citation form throughout.

The two are not in tension. `daniel-voice` is a theory of how sentences connect;
venue convention is a theory of what a section must contain. Flow makes a
conventional paper readable rather than making it unconventional.

Never generate an experimental result, a baseline number, a citation, or a
related-work claim. Draft around gaps and mark them `[TODO: author supplies]`.
