...
This skill provides a systematic framework for strategic planning of academic papers in philosophy and interdisciplinary research. It guides you through three phases, running from platform selection to optimized outline, with AI-driven literature search, research gap identification, originality assessment, and quality-controlled outline design.
...
## Voice and Venue (read at Phase 1, not later)

**Venue selection overrides this skill's defaults.** Phase 1 below assumes a
philosophy preprint server. If the target is ICRA, IROS, RA-L, or any IEEE
robotics conference, read `references/venue_icra.md` and treat it as the Phase 1
output instead of running platform analysis from scratch. It fixes the page
limit, the section structure, the word budgets, the double-anonymous rules, and
the IEEE AI-disclosure policy.

The outline this skill produces must fit the venue's page limit **at outline
stage**. An outline that budgets 1,500 words for an introduction is not a tight
outline for an 8-page double-column paper; it is a paper that will be desk
rejected or gutted at draft stage.

**Register is not this skill's decision.** `academic-paper-composer` writes the
prose, and `daniel-voice` governs how its sentences connect. Note in the outline
which sections carry argument and which report results, since that split is what
tells the composer where voice has room.
